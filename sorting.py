...
def drawData(data, colourArray):
    ...
    canvas_height = 450
    canvas_width = 870
    x_width = (canvas_width - 20) / len(data)  # Adjust width calculation
    offset = 10
    spacing_bet_rect = 2  # Reduced spacing between rectangles
    nomalised_data = [i / max(data) for i in data]

    for i, height in enumerate(nomalised_data):
...
maxvalue = Scale(root, from_=0, to=100, resolution=1, orient=HORIZONTAL, font=("arial", 14, "italic bold"), relief=GROOVE, bd=2,
                 width=10)
